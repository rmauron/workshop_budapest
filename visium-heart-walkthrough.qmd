...
In this workshop, we will discover the data analysis of Visium data from 10x Genomics. All the code is provided, so you are not asked to code it yourself, but rather read thoroughly the steps that help us load the data, filter the data, process it, cluster. In a second time, if time allow, all the steps are repeated for use case with 3 tissue sections, where complementary and more advanced analyses are suggested. If the time does not allow us to do it together, you are welcome to go through it by yourself and contact me later for questions.

The data is collected following the poly-A capture technology from 10x Genomics. On the plot below is a reminder:

![Visium Poly-A](./data/img/10x_visium_poly_A.jpeg)
...
```{r}
# ---------------------------------------------------------------------------
# Full pipeline, 3 sections. Same steps as above, run end to end.
# ---------------------------------------------------------------------------
...
infoTable <- tibble(samples, imgs, spotfiles, json) |>
  mutate(sample_id = basename(dirname(samples)),
         slide_id  = sample_id) |>
  left_join(metadata, by = "sample_id")     # <-- adjust key to your CSV
...
reduction_use <- "harmony"    # switch to "pca" to see the uncorrected version
dims_use      <- 1:20
```
...
saveRDS(se,      "./checkpoints/3sections_clustered.rds")
saveRDS(detable, "./checkpoints/3sections_markers.rds")

# se <- readRDS("./checkpoints/3sections_clustered.rds")
```