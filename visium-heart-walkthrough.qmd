...
# Step-by-step discovery of Visium data analysis using embryonic heart sections.
...
- Found the genes that define each cluster

### Practical notes

Grey boxes are **code chunks**. Run the chunk your cursor is in with 
`Cmd/Ctrl + Shift +Enter`, or a single line with `Cmd/Ctrl + Enter`. 
Run them in order, top to bottom; several steps overwrite `object` with 
an updated version, so skipping one will produce confusing
errors later. Red text in the console is not always an error; warnings are common and
usually harmless. If something breaks, say so and we will look at it together.
...
Note that when using `SubsetSTData()` we assigned the result of the function to the same original name `object`. That action overwrites the previous state of our object. In case you want to keep the unfiltered and filtered together, should assign the result of the function to a different variable name.

### More systematic filtering

The extra transcripts we sequence and gather in our object do not limit itself to the 4 categories we successfully removed above. Databases allow us to annotate the remaining genes we sequenced into some categories. Here, we will use the *GRCh38-2020-A genome reference* which let us figure out the main categories our biomolecules belongs too.
...
We could decide to remove all spots that have less than a certain number of genes since the section we analyse could have holes or empty regions.
...
The data collected by the array and sequenced after library sequencing are prone to biological and technical variations. This raw data is called "counts" in a `Seurat` object. We need to correct for differences in gene sequences, a per-gene z-score so no gene dominates the PCA, (`ScaleData()`), and in spot differences (`NormalizeData()`) so that the expression is comparable across sample, spots and genes.
...
Here you see in the column `pcw_age` (post concepotional week), that we have 3 sections
of 3 distinct ages. We also know the sex of the embryos, but we will look later how we
could figure it out if we did not have that information.
...
```{r}
ggplot(se@meta.data, aes(nCount_Spatial, fill = sample_id)) +
  geom_histogram(bins = 50) +
  facet_wrap(~ sample_id, scales = "free_y") +
  labs(title = "Library size per spot, by section")

MapFeatures(se, features = "nCount_Spatial", pt_size = 1.5)
```
...
### 7.1 Clustering - uncorrected

```{r}
reduction_use <- "pca"
dims_use      <- 1:20

se <- FindNeighbors(se, reduction = reduction_use, dims = dims_use)
se <- FindClusters(se, algorithm = 1, resolution = 0.8)
se <- RunUMAP(se, reduction = reduction_use, dims = dims_use, seed.use = 42)

DimPlot(se, reduction = "umap", group.by = "seurat_clusters") |
  DimPlot(se, reduction = "umap", group.by = "sample_id")

MapLabels(se, column_name = "seurat_clusters", pt_size = 1.5)
```

We can see that here, spots tend to cluster by their respective origins (the 
section they come from), rather than by their biological similarity


### 7.2 Clustering - corrected

So now we will cluster on top of the corrected PCA representation after
Harmony.
...
We can observe that now, spots from the different section group better by
anatomical and molecular similarity, rather than their origin. These steps
are art, and rather project/question dependent. Over correcting, and one
loses the information that separate 1 section to the other. Under correcting,
and one end up retrieving all the technical/batch noise. 
(`Harmony`)[https://www.nature.com/articles/s41592-019-0619-0] provides
a range of parameters worth checking the day you need to use such batch effect
correction.
...
Here, each cluster share spots coming from our 3 sections, which means that the
...
For that we need to subset each clusters per time points (e.g., `cluster_1_age_1`, `cluster_1_age_2`, etc.) and specify to the `FindAllMarkers()` function to look at each of
these subcategories individually.

Let's try it out:

#### 1. Create the age dependent clusters group as a new metadata column

```{r}
# Extract unique cluster numbers and ages as sorted numeric vectors
clusters <- sort(as.numeric(unique(as.character(se$seurat_clusters))))
ages     <- sort(as.numeric(unique(as.character(se$pcw_age))))

# Build all combinations in numeric order (Cluster primary, Age secondary)
grid <- expand.grid(age = ages, cluster = clusters)
ordered_levels <- paste0(grid$cluster, "_age_", grid$age)

# Create the string column
raw_values <- paste0(se$seurat_clusters, "_age_", se$pcw_age)

# Convert to a factor using only the levels actually present in your data
se$seurat_clusters_age <- factor(
  raw_values, 
  levels = intersect(ordered_levels, raw_values)
)

levels(se$seurat_clusters_age)
```


#### 2. Use this new category to compute DEG

```{r}
# Specify which metadata column to use
se <- SetIdent(se, value = "seurat_clusters_age")
...
# Filter the results table by significance
detable <- detable |>
  filter(p_val_adj < 0.05) |>
  mutate(pct.diff = pct.1 - pct.2)
...
top2 <- detable |>
  ...
  slice_max(pct.diff,  n = 5, with_ties = FALSE) |>
  slice_max(avg_log2FC, n = 2, with_ties = FALSE) |>
  ungroup()

# Plot top2 markers per category
DotPlot(se, features = unique(top2$gene), assay = "Spatial") +
  coord_flip() +
  scale_color_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdBu"))) +
  scale_x_discrete(limits = rev) +
  labs(x = "", y = "", title = "Cluster markers across 3 sections")
