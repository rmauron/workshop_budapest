...
- First, a spot is a fixed position, not a cell: at 55 µm it typically covers a 
handful of cells, so what we measure is a local mixture.
- Second, poly-A capture is indiscriminate: any polyadenylated transcript is 
sequenced, whether or not we are interested in it. A large part of this workshop 
...
When working in R, there is multiple ways to set up your working environment.
For the sake of that workshop, you are provided with a fully working environment 
that contains the code, the data, and the R packages already installed. Below we 
are just specifying R where to look for the installed packages.
...
R packages might be installed, but for us to be able to use them later in the 
analysis, we have to attach them to our session. In R, we do it by using the 
`library()` function. This slightly more advanced way below, allow to call the 
function `library()` in a loop. You could also do: 
...
For instance, if I tell R to sample 100 random cells from my single-cell RNA-seq 
data, everytime I run the function, I will have different randomly picked cells. 
It does what we ask, but for the sake of reproducibility, we want to have always 
the same cells picked when we run the same code the next day. To do so, we can 
specify a seed by using `set.seed()`. Some functions have that option in the 
function's parameter (worth checking because it might not be used by default), 
but otherwise you can use `set.seed()` with any number you want 
...
Our session is now fully ready to start. We have the packages loaded, we are 
ready to load some data.

To analyse a [Visium](https://www.10xgenomics.com/platforms/visium/product-family?&utm_medium=search&src=search&utm_source=google&lss=google&utm_term=10x%20visium&useroffertype=website-page&utm_content=website-page&utm_campaign=701KW0000019A9zYAE&cnm=701KW0000019A9zYAE&cid=701KW0000019A9zYAE&usercampaignid=701KW0000019A9zYAE&gad_source=1) 
section, we need to load the data. [10x Genomics](https://www.10xgenomics.com/) 
provides a software ([spaceranger](https://www.10xgenomics.com/support/software/space-ranger/latest)) 
that takes the raw sequences and the microscopy images, and generates a 
`count matrix` (gene counts per barcode), and several files that help us align 
the microscopy image to the count matrix.

### How do we load this count matrix in R?

In R, scientists have developed convenience packages to effortlessly load the 
`spaceranger` outputs, store them into an object class, and provide multiple 
functions to analyse, plot and else.

In the single-cell RNA seq world, such a convenience package is `Seurat`, and 
is very well established and maintained by a large team. They are at the origin 
of the object class that we call `Seurat object`.

When Spatial Transcriptomics appeared, there were no such toolkit, so other labs 
put effort into creating such a package. Today we will rely on the [`semla`](https://github.com/spatial-research/semla) package developed in our lab ([Lundeberg](https://www.spatialresearch.org/)). `semla`, under the hood, uses 
the `Seurat object` class, but added a way to register the spatial information 
...
Below we create a table that provides the path so R (and specific packages e.g., 
*semla*) knows where the `spaceranger outputs` are. We have here:

1.  `filtered_feature_bc_matrix.h5` : the count data
2.  `tissue_hires_image.png` : an H&E image that can be aligned to the counts
3.  `tissue_positions_list.csv`: a table that contains the info to align the 
pixel of the image and the Visium spots
4.  `scalefactors_json.json`: a ratio value that help us visualize the image 
and the spots in the right scale.
...
```{r}
#| label: infoTable-print

infoTable
```

We can now use `ReadVisiumData()` function from *semla* package to create an 
R-object with all the data we linked with their paths in the `infoTable`. *semla* 
uses the data structure from the well-known *Seurat* package, originally developed 
to facilitate the analysis of *single-cell RNA-seq* data. *semla* added, on top 
of this architecture, all the spatial information (image, coordinates, etc.) that 
are required for us to know where are the spots and to be able to later use this 
information in analyses and visualization.

```{r}
#| label: ReadVisiumData
...
The print-out gives `features x samples`, i.e. **genes x spots**.

```{r}
#| label: answer-object-dim

dim(object)      # genes, spots
nrow(object)     # genes
ncol(object)     # spots
```

About 36,600 genes for roughly 2,200 spots. That gene count is close to *twice*
the ~19,000 human protein-coding genes, which is expected: poly-A capture also
picks up lncRNAs, pseudogenes, mitochondrial transcripts and antisense
transcripts. We deal with that in the next section.
:::


### What are we actually looking at?

Before we touch the data, it is worth spending two minutes on the tissue. 
Everything we do from here on produces coloured dots on this image, and those 
dots only mean something if you know what is underneath them.
...
- **Trabeculated myocardium** — the sponge-like inner ventricular wall, with deep 
recesses opening into the lumen
...
1. *MYH6* marks the **atria**, *MYH7* the **ventricles** (strongest in the compact
   myocardium). They are largely mutually exclusive, which we can check numerically
   rather than by eye:

```{r}
#| label: answer-myh6-myh7

d <- FetchData(object, vars = c("MYH6", "MYH7"), layer = "counts")
cor(d$MYH6, d$MYH7, method = "spearman")   # negative = complementary domains
```

2. Spots with neither are **not myocardium**: chamber lumens (little tissue at all),
   valve mesenchyme, and the epicardial surface.

3. *TNNT2* covers all myocardium (atria *and* ventricles), *COL1A1* picks out the valve
   region and mesenchyme, *PECAM1* traces endocardium and vessels — a thin outline
   rather than a solid block.

```{r}
#| label: answer-other-markers

MapFeatures(object, features = c("TNNT2", "COL1A1", "PECAM1"),
            slot = "counts", pt_size = 1, ncol = 3)
```
:::
...
We see about twice as many unique genes as there are human protein-coding genes. 
Our data comes from poly-A capture, so every polyadenylated transcript gets 
sequenced — including a lot we did not set out to measure.

Four families are worth removing. All are very highly expressed, so they eat a 
large share of each spot's counts, and all vary for reasons unrelated to the 
anatomy we want to map:
...
This is a project-specific choice, not a rule: if you studied erythropoiesis, 
hemoglobin would be your signal, not your noise.

In *R*, we can use simple `regular expression` or called `regex`(text patterns 
that match) to retrieve these genes:
...
We can get a list of all the other genes (the ones we want to keep), and see the 
difference compared to the original object.
...
```{r}
#| label: answer-undesirable-count

length(undesirable_genes)                    # total flagged
table(sub("^(MT-|HB|RP|MALAT1).*", "\\1", undesirable_genes))   # by family
```

110 genes: 13 mitochondrial, 10 hemoglobin, ~86 ribosomal proteins and MALAT1.
A small number of genes, but a large share of the counts:

```{r}
#| label: answer-undesirable-fraction

sum(LayerData(object, "counts")[undesirable_genes, ]) /
  sum(LayerData(object, "counts"))           # fraction of all counts removed
```
:::

Now we can proceed with a first filtering. We want to remove the undesirable genes, 
meaning we want to keep only all the other genes. We use the function `SubsetSTData()` 
from the *semla* package, which ensure that the spatial information is also modified 
correctly.

```{r}
#| label: gene-filtering-subset

object <- SubsetSTData(object, features = genes_to_keep)
object
```

Note that when using `SubsetSTData()` we assigned the result of the function to 
the same original name `object`. That action overwrites the previous state of 
our object. If you want to keep the unfiltered and filtered versions side by side, 
assign the result to a different variable name.

### More systematic filtering

The extra transcripts we sequence and gather in our object do not limit themselves 
to the 4 categories we successfully removed above. Databases allow us to annotate 
the remaining genes we sequenced into some categories. Here, we will use the 
*GRCh38-2020-A genome reference* which lets us find out which category each gene 
belongs to.

The complete *GRCh38-2020-A genome reference* is quite large so here we provide 
the single file of that reference that you can load in the next chunk.

*You do not need to run the next chunk* This chunk commented out is for you to 
see what has been done from the complete reference to get the file we will use next.

```{r}
#| eval: false
#| label: prepare-ref-genome
...
Here we will use the information of the gene types to keep only the protein 
coding genes. This step is project dependent, in other cases you might want to 
also keep the long-non-coding RNA or any types of immunoglobulin transcripts.

```{r}
#| label: gene-filtering-ref-genome
...
```{r}
#| label: answer-dim-after-filtering

dim(object)     # genes, spots
ncol(object)    # spots: unchanged, we only ever filtered genes
```

Roughly 19,300 genes remain, down from 36,600. **The number of spots has not
changed** — we removed genes, never spots. That is deliberate, and the next
section explains why.
:::

### Some QC plots

We now cleaned the genes that we are not interested in. We can now analyse the 
distributions of these genes with the commonly used histograms, but also spatially.
...
```{r}
#| label: QC-nCount_Spatial
#| fig-height: 6

p1 <- ggplot(object@meta.data, aes(nCount_Spatial)) + geom_histogram()
p2 <- MapFeatures(object = object, features = "nCount_Spatial", slot = "counts", pt_size = 1)

p <- p1 | p2
p
```

We could decide to remove all spots that have less than a certain number of counts 
since the section we analyse could have holes or empty regions.

We would do it by running:

```{r}
#| label: QC-nCount_Spatial-threshold
#| fig-height: 6
...
Before we continue with the processing, we just need to make sure that no spots 
contains exactly 0 count. That would produces `NaN` in some function, then breaking others.

Check:

```{r}
#| label: QC-metadata-check
...
We checked before by printing the outlook of the object, that we have only the 
raw counts from after the `spaceranger` run.

Now, we will need to go through multiple steps:

#### 1. Normalization

Normalize the counts (the amount of RNA and UMIs differ between spots (cells in 
case of single-cell RNA-seq)). The normalization is done per spot and is a log1p 
of a per spot scaled count.
...
Some of the downstream processing are relatively computing costly, so we gain from 
restricting the gene space to only the genes that are the most variable across spots. 
Excluding the invariable genes help for: 

- Improved signal-to-noise ratio 
- Reduced computational requirements

Using the `Seurat` suite, we can do that easily by using the function 
`FindVariableFeatures()`. That function fits a line to the relationship between 
the log(variance) and the log(mean) using the local polynomial regression (loess) 
per gene.

```{r}
#| label: FindVariableFeatures
#| fig-height: 4
...
Scaled data have a mean of 0 and a variance of 1. That means that some genes have 
positive values and some are negative. This scaled data is used mainly when we 
want to further reduce dimension using PCA.
...
Layers(object)
sapply(Layers(object), function(l) dim(LayerData(object, layer = l)))
```

Three layers:

- `counts` — the raw UMI counts, untouched
- `data` — `NormalizeData()`: corrected **per spot** for sequencing depth
- `scale.data` — `ScaleData()`: z-scored **per gene**

Note the dimensions differ. `scale.data` holds only the 3,000 variable features,
which is why asking for a non-variable gene from that layer returns an error.
:::

That is all the transformation that we will perform on our data. Let's visualize 
how different the same gene can look on the different layers;

```{r}
#| label: print-counts-scale-data
...
These 3 steps allowed us to correct for sequencing depths difference in genes 
and in spots. We found the genes that have the most amount of variance in the 
data (the genes that do not change in space in our section are not the most 
interesting), and scaled the data for PCA.

### Clustering

Our data is now ready for downstream analysis. While we could explore individual 
genes of interest, a state-of-the-art approach borrowed from single-cell RNA-seq 
is cluster analysis. The primary goal of clustering is to group data points 
based on overall similarity. In spatial transcriptomics, we group spots that 
share similar gene expression profiles, which helps differentiate spots 
corresponding to distinct anatomical structures or cellular microenvironments.
...
Calculate distances between spots in PCA space to construct a Shared Nearest 
Neighbor (SNN) graph. In that example, we decide to take the first 20 principal 
components.
...
Partition the SNN graph into distinct groups using community detection algorithms 
(e.g., Louvain or Leiden). You see that there is a `resolution` parameter for 
...
Project the clusters onto a 2D layout to inspect global relationships. Here we 
want to visualize each of our spots as clusters that we just computed. So we 
take the first 20 principal components from our PCA analysis, and project them 
in a 2D space using the **Uniform Manifold Approximation and Projection (UMAP)**, 
"a popular data science tool used for machine learning and data visualization. 
It shrinks complex information with many variables down into a simple 2D or 3D 
picture."
...
Once UMAP is ran, we can visualize the results in a scatter plot. We need to 
specify the `metadata` column that contains the label of our clusters. In that 
case, we use the column called `seurat_clusters`.
...
Overlay cluster colors directly onto the histology image to verify anatomical 
alignment.
...
We would like now to identify which genes are the most specific to each of the 
clusters we identified. To do so we use an analysis method called 
`Differentially Expressed Genes` or `DEG`.

We rely mainly on the `FindAllMarkers()` function from the `Seurat` package. 
If you check the function documentation online, you can find out about the 
different test that one can choose from. We will use the default `Wilcoxon test`. 
The function accounts for the multiple testing. After running the test, we will 
explore quickly its results.
...
We can see that the results is a dataframe containing several metrics in the 
columns such as the average log fold change (avg_log2FC) or the adjusted p-values 
(p_val_adj), but also the name of the genes and the clusters it belongs to.
We expect to recover known anatomical markers since the test is supposed to pull 
out which genes are the most specific to our clusters. 
...
Yes — and the odd thing is that **the same gene appears in several clusters**.

```{r}
#| label: answer-detable-duplicates

nrow(detable)
detable |> count(gene, sort = TRUE) |> head(5)      # genes flagged most often
detable |> filter(gene == "MYH6") |> select(cluster, avg_log2FC, p_val_adj)
```

`FindAllMarkers()` tests each cluster against **all others pooled**, one cluster at
a time. A gene can therefore be significantly enriched in cluster 2 *and* cluster 5,
as long as each is higher than the rest combined. With several clusters covering
myocardium, *MYH6* comes out repeatedly. This is why we add `pct.diff` next: it
rewards genes that are *specific*, not merely high.
:::
...
We can see that the dimension of the table is quite large, which means that some 
genes appear in multiple clusters. We also see that when checking all the rows 
that contain the gene `MYH6`.
...
# gene → structure lookup, de-duplicated
g2s <- stack(reference_markers) |>
  setNames(c("gene", "structure")) |>
  dplyr::filter(!duplicated(gene))

p <- DotPlot(object, features = as.character(g2s$gene), assay = "Spatial")

d <- p$data
d$structure <- factor(g2s$structure[match(d$features.plot, g2s$gene)],
                      levels = names(reference_markers))

ggplot(d, aes(x = id, y = features.plot,
              size = pct.exp, colour = avg.exp.scaled)) +
  geom_point() +
  facet_grid(structure ~ ., scales = "free_y", space = "free_y") +
  scale_colour_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdBu")),
                         name = "Avg. expression") +
  scale_size(range = c(0, 6), name = "% expressed") +
  labs(x = "cluster", y = NULL,
       title = "Our clusters against published structural markers") +
...
Passing a named list to `DotPlot()` groups the genes by structure, so each block 
of columns is one candidate identity. A cluster with a strong, isolated block is 
straightforward to name. A cluster with moderate signal in several blocks is 
probably a mixture — which is expected, since a 55 µm spot rarely contains only 
one cell type.
...
MapLabels(object, column_name = "manual_annotation", pt_size = 1.5)
```
...
2. The atlas resolved 23 spatial clusters. How many did we get, and what would you 
   change to get closer?
...
1. The abundant structures are assignable: atrial and ventricular myocardium, valve
   mesenchyme, epicardium, endocardium. The rare ones are not.

2. How many did we get, and how big is each?

```{r}
#| label: answer-cluster-count

nlevels(object$seurat_clusters)
sort(table(object$seurat_clusters))       # spots per cluster
```

To get closer you would need more sections, more spots, and a higher `resolution`
— but resolution alone cannot invent signal that is not there.

3. Because a cluster needs spots to exist. The smallest structures cover only a
   handful of 55 µm spots in any one section:

```{r}
#| label: answer-smallest-cluster

min(table(object$seurat_clusters))        # our smallest cluster
round(100 * min(table(object$seurat_clusters)) / ncol(object), 1)  # % of section
```

The SA node is smaller than that. Louvain has nothing to find.

4. Open the viewer and compare — most abundant-structure markers will agree, which
   is a good sign that our coarse clustering is not wrong, just incomplete.
:::
...
Yes. `XIST` is high only in the female section; the Y-linked genes are present only
in the male ones. We can make the call explicit and compare it to the metadata:

```{r}
#| label: answer-sex-call

FetchData(se, layer = "counts", vars = intersect(sex_genes, rownames(se))) |>
  mutate(sample_id = se$sample_id) |>
  group_by(sample_id) |>
  summarise(across(everything(), mean)) |>
  mutate(predicted = ifelse(XIST > DDX3Y, "female", "male")) |>
  left_join(infoTable |> select(sample_id, sex), by = "sample_id") |>
  select(sample_id, XIST, DDX3Y, predicted, reported = sex)
```

Note that this only works **before** filtering: `XIST` is a lncRNA and would be
removed by the protein-coding filter, and `RPS4Y1` is caught by our ribosomal
pattern.
:::


```{r}
#| label: sex-table-3sect
...
p2 <- MapFeatures(se, features = "nCount_Spatial", pt_size = 1, ncol = 3, label_by = "sample_id")
...
```{r}
#| label: cluster-table-3sect

table(se$clusters_pca, se$clusters_harmony)
```

A cluster whose spots spread across many columns was split or reassigned by the 
correction.
A tight diagonal block means that cluster was robust either way.

#### Back to the tissue

The UMAP is a summary; the section is the ground truth. A good correction should 
make the same anatomical structure receive the same cluster label in all three 
sections.

```{r}
#| label: clusters_pca_map-3sect
#| fig-width: 12
...
```{r}
#| label: clusters_harmony_map-3sect
#| fig-width: 12
...
1. Before correction, many clusters are dominated by a single section. We can count
   them rather than squint at the UMAP — for each cluster, what share of its spots
   comes from its most common section?

```{r}
#| label: answer-cluster-purity

purity <- function(clusters, samples) {
  tab <- table(clusters, samples)
  round(apply(prop.table(tab, 1), 1, max), 2)
}

p_before <- purity(se$clusters_pca,     se$sample_id)
p_after  <- purity(se$clusters_harmony, se$sample_id)

c(n_clusters_before = length(p_before),
  n_clusters_after  = length(p_after))

summary(p_before)
summary(p_after)
```

A value near 1 means "this cluster is essentially one section". Count how many
clusters cross 90%:

```{r}
#| label: answer-cluster-purity-count

c(before = sum(p_before > 0.9),
  after  = sum(p_after  > 0.9))
```

2. After correction the ventricular wall should carry the same label in all three
   sections. Before, it typically has one label per section — the same biology
   split three ways by slide.

Careful though: with one section per age, some of what Harmony removed is real
developmental difference. See the limitation box below.
:::
...
```{r}
#| label: sample_id_age_table-3sect
...
```{r}
#| label: answer-1-vs-3-sections

c(one_section    = nlevels(object$seurat_clusters),
  three_sections = nlevels(se$clusters_harmony))

c(spots_one   = ncol(object),
  spots_three = ncol(se))
```

Three reasons for the difference:

- **More spots** means more statistical power, so structures too small to separate
  in one section can now form their own cluster.
- **Three developmental stages** are pooled, so a cluster must describe a structure
  across PCW 6 to 10 rather than at a single moment.
- **Harmony** reshaped the embedding, and cluster numbering is arbitrary anyway —
  cluster 3 here has nothing to do with cluster 3 in part 1.

The markers are the reliable comparison, not the labels:

```{r}
#| label: answer-marker-overlap

top_now <- detable |> group_by(cluster) |> slice_max(avg_log2FC, n = 10) |>
  pull(gene) |> unique()
length(top_now)
```
:::
...
We just did a `DEG` analysis on the 3 sections together, but an information we 
do not know yet is what markers are specific `per clusters` **AND** `time point`. 
For that we need to subset each clusters per time points individually and repeat 
`FindAllMarkers()` function to look at each of the time point separately.
...
```{r}
#| label: check-clusters-table-3sect
...
```{r}
#| label: temporal-deg-violinplot-3sect
#| fig-height: 8
#| fig-width: 6
...
```{r}
#| label: temporal-deg-dotplot-3sect
#| fig-height: 4
#| fig-width: 5
...
```{r}
#| label: CorSpatialFeatures-3sect
...
```{r}
#| label: CorSpatialFeatures-1-pos-3sect
#| fig-height: 7
...
```{r}
#| label: CorSpatialFeatures-1-neg-3sect
#| fig-height: 7
...
```{r}
#| label: CorSpatialFeatures-2-pos-3sect
#| fig-height: 7
...
```{r}
#| label: CorSpatialFeatures-2-neg-3sect
#| fig-height: 7
...
```{r}
#| label: CorSpatialFeatures-3-pos-3sect
#| fig-height: 7
...
```{r}
#| label: CorSpatialFeatures-3-neg-3sect
#| fig-height: 7
...
In this workshop we went through the most important pre-processing, processing, 
clustering steps for a single section and for a couple of sections together. 
We explored some quality metrics, tried to correct for batch effect using Harmony, 
and explored a way to identify temporal changes across clusters of interest. 
Finally we understood that this is just the start, and from there much more
spatially aware analyses can be performed.

Due to lack of time, we have not covered deconvolution, niche analysis, pathways, 
spatially aware clustering, spatial interactions, pseudotime analysis, and more.
...
- [Seurat guided clustering](https://satijalab.org/seurat/archive/v3.0/pbmc3k_tutorial.html)
- [Seurat integration & label transfer](https://satijalab.org/seurat/archive/v3.2/integration.html)
- [Single-cell QC & Clustering](https://rnabio.org/module-08-scrna/0008/02/01/QA_clustering/)
- [NBIS detailed slides](https://nbisweden.github.io/workshop-scRNAseq/home_contents.html)
