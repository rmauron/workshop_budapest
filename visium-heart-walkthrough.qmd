...
object$manual_annotation <- annotation[as.character(object$seurat_clusters)]
MapLabels(object, column_name = "manual_annotation")
```
...
res |> group_by(cluster) |> slice_max(avg_log2FC, n = 10) |> ungroup()
```


By doing this, we isolate all the spots corresponding to a cluster of interest and ask
"Whithin this cluster, which are the marker that differs the most accross age/section?".
The way it is calculated is, taking the spots of the clusters of choice of 1 age (e.g. pcw 6)
and compare it to the spots of age 2 and 3 together (e.g. pcw 9 and 10 pulled together).
...
```{r}
VlnPlot(sub, features = head(unique(res$gene), 6), group.by = "pcw_age", ncol = 3)
```

```{r}
# Plot top2 markers per category
DotPlot(sub, features = head(unique(res$gene), 6), assay = "Spatial") +
  coord_flip() +
  scale_color_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdBu"))) +
  scale_x_discrete(limits = rev) +
  labs(x = "", y = "", title = "Cluster markers across 3 sections")
...
MapFeatures(
  object = se,
  features = head(unique(res$gene), 6),
  ncol = 3
)
```


### 11. How to take of the spatial information bioinformatically

We covered many steps until here. Many of the steps that are covered in most Visium
data analysis. But it is important to note that most of these steps are borowed from 
...
"Does the genes I considered Variable Gene, are actually spatially specific?"

A gene might have an intrisic high variability accross spots, but its expression be 
completely scattered accross the tissue section. We can rely on some spatial 
...
```{r}
positive_corr <- sp_cor[[1]]$gene[1:5]
...
```{r}
negative_corr <- tail(subset(sp_cor[[1]], !is.nan(cor))$gene, 5)
