...
res <- as_tibble(res)
res <- res |> dplyr::rename(age = cluster) |> group_by(age) |> slice_max(avg_log2FC, n = 10) |> ungroup()
```
...
VlnPlot(sub, features = top2_per_age$gene, group.by = "pcw_age", ncol = 2)
```

```{r}
# Plot top2 markers per category
DotPlot(sub, features = top2_per_age$gene, assay = "Spatial") +
  coord_flip() +
  scale_color_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdBu"))) +
  scale_x_discrete(limits = rev) +
...
clustering, spatial interactions, pseudotime analyzis, and more.
...
[Seurat documentation](https://satijalab.org/seurat/articles/spatial_vignette)
...
[Spatial interaction](https://github.com/StatBiomed/SpatialDM)
